...
def plotHistogram(label_data):
    ...
    plt.figure()
    plt.bar(range(len(classes_counts)), list(classes_counts.values()), align='center')
...
def generateRotatedImage(image):
    ...
    M = cv2.getRotationMatrix2D((image_shape[0]/2,image_shape[1]/2),degree,1)
...
if not  os.path.isfile('trainAugmented.p'):
    ...
    new_pop = random.choices(list(inverse_train_prob.keys()), inverse_train_prob.values(), k=20000)
...
for i in range(n_validation):
    X_valid_grey[i] = clahe.apply(cv2.cvtColor(X_valid[i][:32] , cv2.COLOR_RGB2GRAY), 0)
...
def LeNet(x):    
    ...
    conv1_W = tf.Variable(tf.truncated_normal(shape=(5, 5, image_shape[2], 32), mean = mu, stddev = sigma))
...
# SOLUTION: Layer 3: Convolutional. Output = 10x10x16.
    ...
    fc1_W = tf.Variable(tf.truncated_normal(shape=(14336, 2048), mean = mu, stddev = sigma))
...
with tf.Session() as sess:
    ...
    saver.save(sess, './trafficSignModel01')
    print("Model saved")